...
(function initCookieBanner() {
  ...
  if (!banner || !wheel || typeof gsap === "undefined") return;
  ...
  // Calcular distancia real de rodado en el momento que se crea la timeline
  const travelX = banner.offsetWidth - wheel.offsetWidth - 80;
  ...
  tl.to(wheel, { x: travelX, rotation, ease: "none" }, 0)
    .to(text,  { opacity: 1, x: 0, ease: "none" }, 0.1); // leve retraso para que la cookie llegue primero

  function dismiss() {
    gsap.to(banner, { height: 0, opacity: 0, duration: 0.4, ease: "power2.in",
      onComplete: () => banner.classList.add("is-dismissed") });
  }
